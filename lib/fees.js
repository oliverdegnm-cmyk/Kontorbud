...
// Viser et beløb pænt med "DKK" bagved, uden at duplikere valuta-tekst hvis
// brugeren allerede selv har skrevet en (f.eks. "1000 kr" eller "Ikke angivet").
export function formatBudgetDisplay(value) {
  if (!value) return value;
  const trimmed = value.toString().trim();
  const hasCurrencyAlready = /kr|dkk/i.test(trimmed);
  const isNumericLike = /\d/.test(trimmed);
  if (!isNumericLike || hasCurrencyAlready) return trimmed;
  return `${trimmed} DKK`;
}
