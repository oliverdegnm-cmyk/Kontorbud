...
const EMAIL_SUBJECTS = {
  ...
  new_message: "Ny besked på AIbud",
...
export async function notify(recipientName, type, taskId, body, origin) {
  ...
  try {
    ...
    if (user?.email_verified && user?.email_notifications) {
      const siteUrl = origin || process.env.NEXT_PUBLIC_SITE_URL || "https://aibud.dk";
      await sendNotificationEmail(user.email, recipientName, EMAIL_SUBJECTS[type] || "Notifikation fra AIbud", body, siteUrl);
    }
  } catch (err) {
